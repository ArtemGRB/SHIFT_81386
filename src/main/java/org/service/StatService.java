...
public class StatService {
    ...
    public void printFullStat() {
        ...
        if (!inMemoryRepository.getStringList().isEmpty()) {
            ...
            System.out.println("  Минимальная длина: "
                    + Collections.max(inMemoryRepository.getStringList(), Comparator.comparingInt(String::length)).length());
            System.out.println("  Максимальная длина: "
                    + Collections.min(inMemoryRepository.getStringList(), Comparator.comparingInt(String::length)).length());
        }
    }
    ...
    private BigDecimal calculateAverage(List<String> numbers) {
        int scale = 3;
        if (numbers.isEmpty()) {
